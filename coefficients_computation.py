...
def compute_coefficients(type_exp):
    ...
    surface_v_to_volume = 3.0  # mm^-1
    surface_a_to_volume = 3.0  # mm^-1
    surface_c_to_volume = 9.0  # mm^-1
    ...
    B_blood = 116.0 * 1.0e3 / 100 * 2.0 / 60  # flow in blood network
    ...
    B_s = (
        2.5e-6
    )  # Width of the small slit of the tight junction strand (Allt and Lawrenson, 1997; Cassella et al., 1997)
    beta = a_AB / B_s
    D_sl_AB = D_Free_AB * (
        1 - 2.10444 * pow(beta, 6) + 2.08877 * pow(beta, 3) - 0.094813 * pow(beta, 5) - 1.372 * pow(beta, 6)
    )
    ...
    volume_fraction_of_blood = 3.29 / 100.0
    arteries_frac = 0.21
    veins_frac = 0.46
    capillaries_frac = 0.33
    PVS_fraction = 1.0 / 100.0
    ...
    if type_exp == "ECS enhanced":
        ...
        main_dict["kappa_pa"] = kappa_pa * 16.0
        main_dict["kappa_pv"] = kappa_pv * 16.0
